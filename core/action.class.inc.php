...
EOF
	...
	{
		$aEmailRecipientsOQL = [];
		$sEmailRecipientsOQL =  "SELECT EmailRecipients AS recip JOIN lnkEmailRecipientsToActionEmail AS lnk ON lnk.emailrecipients_id=recip.id WHERE lnk.action_id=:action_id AND lnk.field_type=:field_type";
		$oEmailRecipientsSet =  new DBObjectSet(DBObjectSearch::FromOQL($sEmailRecipientsOQL), array() /* order by*/, array('action_id'=> $this->GetKey(),'field_type' => $sRecipAttCode ));
		while ($oEmailRecipients = $oEmailRecipientsSet->Fetch())
		{
			if($sCurrentOQL = $oEmailRecipients->Get('oql')){
                $aEmailRecipientsOQL[] = $sCurrentOQL;
			}
		}
